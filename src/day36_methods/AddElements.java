...
public class AddElements {
    ...
    public static int[] addElement(int[] original, int elementToAdd) {
        int[] newArray = new int[original.length + 1];//creating a blank new array with a length of one more than that of the original one
        for (int i = 0; i < original.length; i++) {
            newArray[i] = original[i]; // copying original array into the new array
        }
    ...
    public static int[] addElement(int[] original, int[] elementsToAdd) {
        ...
        for (int i = 0, j = 0; i < newArray.length; i++) {
            ...
            if (i < original.length) {
                //added elements from the original array in the new array
